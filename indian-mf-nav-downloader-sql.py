...
conn = sqlite3.connect(sys.path[0] + os.path.sep + "all-funds-nav.sqlite")
c = conn.cursor()
...
def add_funds_to_file(fp, mfid, scid):
    ...
    for element in table: 
        ...
        if (len(lines) != 0) and lines[0] != '\n':
            ...
            c.execute('INSERT INTO NAV VALUES (?, ?, ?, ?, ?, ?)', (lines[-1], fund_house_name[mfid], mfid, scheme_name[scid], scid, lines[0]))
...
try:
    with open(csv_path, 'w') as fp:
        ...
        mfdone = 0
        c.execute('CREATE TABLE IF NOT EXISTS NAV (NAVDATE DATE, fund_house TEXT, mfId INT,scheme TEXT,scid INT,nav REAL)')
        conn.commit()
